...
- [x] Real PostgreSQL 18 tests cover first write, revision, immutable history, stale-write conflict, retry collision and cross-Organization isolation. — NOT executed locally in this session (local `postgres` superuser credentials unavailable — `pg_hba.conf` requires `scram-sha-256`; no brute-forcing/config change attempted, matching SLICE-0016 precedent). Verified instead by observing the pushed branch's GitHub Actions `db-integration` job on exact HEAD `ee40b85`: all 20 tests in `tests/persistence/test_native_listing_offer_persistence.py` passed against real PostgreSQL 18 (272/272 in `tests/persistence/` overall). CI run: https://github.com/StradaDelSole/HullQ/actions/runs/33918888424 (job `db integration (PostgreSQL 18)`).
- [ ] Owner inspection ends `PASS` and demonstrates the required end-to-end cases. — script written (`scripts/inspect_native_listing_offer_facts.py`) but NOT executed in this session or by CI (the `db-integration` CI job runs `pytest tests/persistence/` only, not this script). Every scenario the script demonstrates has an equivalent dedicated real-PostgreSQL test that did pass in CI (see above), but the standalone consolidated inspection run itself remains unverified. Reviewer/owner should run it locally against PostgreSQL 18 before acceptance.
- [x] Full repository test/quality/security gates pass. (full suite 4019 passed/274 skipped locally; ruff format/check; mypy; repository validation; `uv lock --check`; `pip-audit`; CI `quality`/`dependency audit` jobs on HEAD `ee40b85`)
- [x] GitHub Actions CI passes on the exact final implementation HEAD. — HEAD `ee40b85`, run https://github.com/StradaDelSole/HullQ/actions/runs/33918888424 (db-integration, quality ubuntu/windows, dependency audit all PASS).
- [x] Manufacturer artifact reproducibility passes on the exact final implementation HEAD. — HEAD `ee40b85`, run https://github.com/StradaDelSole/HullQ/actions/runs/33918888412 (reproduce ubuntu/windows both PASS).
