...
- [x] Real PostgreSQL 18 tests cover first write, revision, immutable history, stale-write conflict, retry collision (same-listing and, per the amendment review, cross-listing) and cross-Organization isolation, plus the amendment-added exact predecessor chain, composite-FK same-listing integrity, NULL-hole and non-finite CHECK-constraint adversarial cases, and equivalent-Decimal-representation idempotency. — NOT executed locally in this session (local `postgres` superuser credentials unavailable — `pg_hba.conf` requires `scram-sha-256`, and this session confirmed it cannot reload PostgreSQL config without Windows service-control/admin rights, so this is a structural block, not merely an unknown password). Verified instead by observing the pushed branch's GitHub Actions `db-integration` job on exact HEAD `4fa6da1`: all 287 tests in `tests/persistence/` passed against real PostgreSQL 18. CI run: https://github.com/StradaDelSole/HullQ/actions/runs/33926347812 (job `db integration (PostgreSQL 18)`).
- [x] Owner inspection ends `PASS` and demonstrates the required end-to-end cases. — Actually executed against real PostgreSQL 18 and observed ending `NATIVE LISTING OFFER FACTS RESULT -> PASS`, via a temporary CI step added to `.github/workflows/ci.yml` for this single verification run (since no local PostgreSQL credentials were available), then removed once the PASS output was captured; `.github/workflows/ci.yml` on the exact final HEAD below is byte-identical to `origin/main`'s. CI run that produced the PASS: https://github.com/StradaDelSole/HullQ/actions/runs/33926347812 (job `db integration (PostgreSQL 18)`, step "SLICE-0045 owner inspection (temporary verification run)"), HEAD `4fa6da1`.
- [x] Full repository test/quality/security gates pass. (full suite locally: 4027 passed/289 skipped; ruff format/check; mypy; repository validation; `uv lock --check`; `pip-audit`; CI `quality`/`dependency audit` jobs)
