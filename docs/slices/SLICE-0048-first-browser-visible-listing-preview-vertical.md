...
- token is URL-safe;
- finite expiry is mandatory;
- default proof TTL may be 24 hours;
- operator override, if supported, is capped at seven days;
...
A preview token is a bearer capability: possession grants preview access until expiry. It MUST therefore be treated as a secret even though it appears in a shareable URL.

Token values and signing secrets MUST NOT appear in ordinary application logs, access logs, tracebacks returned to clients, analytics events or retained test artifacts.

Because the preview token is carried in the URL path for SSR/shareability, default unredacted request-path access logging is forbidden on token-bearing preview routes. In particular, ordinary Uvicorn access logging MUST NOT remain enabled in a form that records the complete preview request path. The implementation MUST either disable such access logging for the preview runtime/proof or redact the token before any request-path log is emitted. The Astro preview server must provide equivalent protection.

0048 preview responses MUST send, at minimum where applicable:
...
The HTML preview MUST contain an equivalent robots meta directive. The preview page MUST NOT load third-party analytics, trackers or other third-party subresources in 0048, because the token-bearing URL must not be leaked through referrers or telemetry.

No canonical link, sitemap entry or public internal-link architecture may point at preview URLs.
...
```text
asking_price_mode
asking_price_amount (lossless decimal/string representation; never binary float)
currency
location_country
...
Explicit `UNKNOWN` remains unknown with the same attribution/time/verification-none disclosure.

Never render `VAT paid ✓`, `verified VAT`, `HullQ verified` or equivalent unless a later accepted verification capability exists.
...
2. create/ensure PhysicalBoat (prefer `BoatDesignRef = NONE` unless a real canonical fixture is required);
...
6. serve FastAPI preview endpoint with token-safe logging configuration;
7. serve Astro SSR preview surface with token-safe logging configuration;
...
12. assert captured ordinary API/web/application logs do not contain preview token or signing secret;
13. end:
...
Cover happy path; exact idempotent retry; PhysicalBoat conflict; missing BoatDesign; MarketEpisode conflict/missing PhysicalBoat; denied publisher; NativeListing collision; different existing offer head under first-revision-only intake; retry after partial durable progress; and no token emitted on failure.

### Token/security

Cover valid round-trip; expiry; max TTL; tamper; malformed token; wrong secret; invalid base64url secret; decoded secret shorter than 32 bytes; constant-time comparison path; token/secret absent from ordinary app/access logs; `Cache-Control: private, no-store`; `Referrer-Policy: no-referrer`; robots protections; and no third-party preview-page subresources.

### API/read model

Cover explicit current-head use; invalid/expired/nonexistent/incomplete all externally not-found-equivalent; unresolved listing (`market_episode_id = NONE`) not previewable; no current offer not previewable; decimal lossless/no float; omission/assertion distinctions; conservative history language; required VAT qualification/attribution/time/verification-none; and no internal metadata exposure.

### Web

Cover Astro check/build; valid SSR preview content from FastAPI; visible preview-only status; conservative VAT/history wording; noindex/nofollow/noarchive; no-store/no-referrer behavior; invalid preview 404/not-found; no direct DB access; token-safe web logs.
...
- publish/unpublish/withdraw/sold workflow;
...
- dedup/merge engine;
- LLM extraction;
- deployment automation/VPS/Cloudflare/DNS changes;
- mobile app.

## 14. Expected touch points

Likely smallest coherent implementation set:
...
8. token-bearing responses are no-store/no-referrer/noindex as specified and preview page has no third-party telemetry/subresources;
...
15. Astro SSR reads through FastAPI only;
16. real HTTP/browser preview visibly renders persisted listing;
17. preview is clearly not published and mechanically non-indexable;
18. stable API/canonical URL/i18n/publication semantics are not silently frozen;
19. dependencies are locked/reproducible; no unjustified migration;
20. existing Python CI/coverage/replay/manufacturer gates remain intact;
21. deterministic web gates are green;
22. retained inspection ends `FIRST VISIBLE LISTING PREVIEW RESULT -> PASS`;
23. exact-head independent review has no material finding;
24. explicit Project Owner acceptance occurs before merge.
