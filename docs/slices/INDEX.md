...
**Updated:** 2026-08-25 (SLICE-0023 implementation complete — `REVIEW`, pending independent review / owner acceptance)
...
| SLICE-0022 | IMPLEMENTATION | DONE | Offline Tier-0 admission-safety pilot over the exact 57 retained SLICE-0021 alternative-route candidates | SLICE-0021 accepted / DONE |
| SLICE-0023 | DESIGN_RESEARCH | REVIEW | Bounded English-Wikipedia category identity-lead discovery pilot (`Category:Keelboats`/`Catamarans`/`Trimarans`) measuring research-lead yield/noise beyond the accepted Stage-3.2 Wikidata boundaries | SLICE-0022 accepted / DONE |

## Current execution rule

**SLICE-0001 through SLICE-0022 are accepted / `DONE`. SLICE-0023 implementation is complete and handed off as `REVIEW`; it is not yet accepted/`DONE`.** SLICE-0022 is closed by `docs/slices/SLICE-0022-acceptance-closure.md` after independent-review `ACCEPT`, exact-head CI success, PR #55 merge and explicit project-owner acceptance on 2026-08-25.

## SLICE-0023 — implementation complete, `REVIEW` (not yet accepted)

SLICE-0023 is a bounded `DESIGN_RESEARCH` pilot measuring whether a fixed, bounded English-Wikipedia category-discovery path (`Category:Keelboats`, `Category:Catamarans`, `Category:Trimarans`, `list=categorymembers` main-namespace only, no recursion) can provide a materially useful set of sailboat model/class research leads beyond the accepted Stage-3.2 Wikidata boundaries, while keeping Wikipedia/Wikimedia content strictly outside canonical admission and production-value ingestion. It is not a production adapter, not a canonical bootstrap expansion and not Stage-3.3 technical enrichment.

The one bounded live acquisition run (implementation-agent local measurement, not yet independently re-verified) found: accepted immutable boundaries reproduced exactly (1,829 direct-discovery QIDs / 1,770 canonical BoatModels / 1,772 historical crosswalk mappings / 57 SLICE-0021 alternative-route union, all fail-closed asserted before acquisition); category membership Keelboats=930, Catamarans=116, Trimarans=86 (all `COMPLETE`, none `CAP_EXCEEDED`); 1,131 unique pages (1 cross-category duplicate page ID, 0 duplicate QIDs); overlap categorization `accepted_direct_qid_overlap`=717, `retained_alternative_qid_overlap`=4, `incremental_qid_lead`=409, `no_wikidata_qid`=1; exact trim+casefold-only title-signal probe found 0 `exact_signal_other_qid`, 414 `no_exact_signal`, 0 `unresolved_structural`; deterministic SHA256-ordered quality sample selected the full 150-QID cap (30 Trimarans / 30 Catamarans / 90 Keelboats, no cross-stratum backfill); manual research-only quality review of all 150 sampled incremental QIDs (using only retained Wikidata CC0 label/description/P31/P176/P287 context, never article prose) found 106 `plausible_model_or_class_lead` (70.67%), 19 `obvious_out_of_scope` (12.67%), 25 `ambiguous` (16.67%); total external HTTP requests 30 (27 Wikipedia + 3 Wikidata), well within the fixed <=75/<=10/<=85 ceilings. Applying the precommitted mechanical rule (incremental count >=100 and >=50% plausible) yields **`FOLLOWUP_VERIFICATION_CANDIDATE`**.

Retained package: `research/bootstrap/wikimedia/sl0023-category-leads/` (`source_assessment.json`, `discovery_manifest.json`, `quality_sample.json`, `REPORT.md`, `ARTIFACT-DIGESTS.json` plus their JSON schemas). The retained package passes its own strict offline `--verify` recompute/tamper-check with zero mismatches. No canonical Brand/Organization/BoatModel/BoatDesign row was created, modified or deleted; no HullQ ID was minted; the accepted SLICE-0017/0018/0021 retained artifacts were read-only inputs and remain byte-unchanged; production Wikidata discovery is unchanged and Wikipedia/Wikimedia was not added to production discovery. This entry records the implementation agent's own measurement and does not constitute independent review or project-owner acceptance; `FOLLOWUP_VERIFICATION_CANDIDATE` is a research recommendation only and does not authorize production acquisition, canonical admission, Stage-3.3 enrichment or SLICE-0024.

SLICE-0022 was deliberately offline and bounded to the exact accepted 57-candidate SLICE-0021 retained set: 53 R1 candidates and four R3 repair signals. It reused the accepted SLICE-0017/0018 Tier-0 admission, collision, provenance and PostgreSQL replay boundaries, performed zero live Wikidata acquisition and did not change HullQ's production Wikidata discovery query.
...
SLICE-0022 classified all 57 alternative-route QIDs under the accepted Tier-0 admission boundary **as controlled by the R1 admission governance amendment** (`docs/slices/SLICE-0022-r1-admission-governance-amendment.md`): a first implementation pass auto-admitted 27 R1 candidates under the unmodified SLICE-0017/0018 label/collision rule, but independent review found a retained counterexample (`Q232393`, "Zweier-Canadier", a German canoe-class term whose only distinguishing structured fact from legitimate Olympic sailing-class candidates was free-text description — not a safe automatic-admission signal) proving R1 route membership alone is discovery-authoritative but not admission-authoritative. The governance amendment authorizes exactly one new rule: R1 route membership can never itself produce `AUTO_ADMIT`. The accepted result is **0 AUTO_ADMIT**, **31 REVIEW_REQUIRED** (27 R1 with reason `r1_alternative_route_requires_review`, 4 R3 with reason `r3_repair_signal_requires_review`, neither ever auto-admitted), **26 NOT_ADMITTED** (missing label), **0** search-projection collisions against the accepted baseline or within the 57. `no_exact_identity_signal` (the SLICE-0021 probe) never itself authorized any outcome — it was only the retained input. R1/R3 `FOLLOWUP_DISCOVERY_CANDIDATE` dispositions remain recommendation-only and do not authorize production route adoption.

The accepted SLICE-0018 measurement remains an immutable input boundary: **1,829** unique direct-instance Wikidata sailboat-class QIDs, **1,770** accepted combined sparse canonical BoatModels and **1,772** retained historical QID→HullQ-ID mappings. Because SLICE-0022 admits zero new candidates from either route, its retained crosswalk remains byte-identical to the accepted combined 1,772-entry baseline crosswalk, and offline-verified PostgreSQL replay of the combined 1,829/1,770/1,772 baseline plus the SLICE-0022 delta reaches a combined canonical BoatModel count of exactly **1,770** — measured locally against PostgreSQL 18 and independently reproduced by exact-head remote CI.

No production Wikidata query change, manufacturer archive ingestion, broad technical enrichment, review-queue campaign, query engine, API, frontend, marketplace or monitoring work is authorized by SLICE-0022's acceptance closure.

## SLICE-0022 acceptance closure

SLICE-0022 is explicitly accepted and `DONE`.

Acceptance evidence:

- final independent-review verdict: **ACCEPT**;
...
- exact-head CI run ID `32843428842`: SUCCESS — quality (ubuntu-latest), quality (windows-latest), db integration (PostgreSQL 18), dependency audit all SUCCESS;
- immutable baseline assertions: **1,829** candidate QIDs / **1,770** canonical BoatModels / **1,772** historical QID→HullQ-ID mappings;
- exact retained candidate universe: **57** unique QIDs = **53 R1 / 0 R2 / 4 R3**;
- accepted final decisions: **0 AUTO_ADMIT / 31 REVIEW_REQUIRED / 26 NOT_ADMITTED**;
- R1 result: **27 REVIEW_REQUIRED / 26 NOT_ADMITTED**, with no R1 auto-admission;
- R3 result: **4 REVIEW_REQUIRED**, with no R3 auto-admission;
- `Q232393` remains review-bound with `hullq_id: null`;
- retained crosswalk remains exactly **1,772** and canonical BoatModel universe remains exactly **1,770**;
- PostgreSQL replay/re-import/fresh-schema proof: zero conflicts, errors, stray rows or semantic drift, `all_zero_tolerance_conditions_clear = true`;
- final amendment added fail-closed retained `REPLAY-RESULT.json` / `REPLAY-REPORT.md` verification before any fresh replay may overwrite them, with pre-`psycopg.connect()` tamper rejection;
- zero live external acquisition;
- production Wikidata discovery query unchanged;
- explicit project-owner acceptance on **2026-08-25**.

Final closure record: `docs/slices/SLICE-0022-acceptance-closure.md`.

No SLICE-0023 or later slice is made `READY` by the SLICE-0022 closure.

## SLICE-0021 acceptance closure

SLICE-0021 is explicitly accepted and `DONE`.

Acceptance evidence:

- independent-review final verdict: **ACCEPT** after two amendment rounds;
- implementation/research PR #50;
- final reviewed / accepted head: `2cf0ab437d2347a574fd5a01b3e5577ca4c6b521`;
- implementation/research merge commit: `c562b1c8f082f60823e084b1fb24c9f5c6f6ba00`;
- exact-head CI run #260 (`32759454547`): SUCCESS — quality (ubuntu-latest), quality (windows-latest), db integration (PostgreSQL 18), dependency audit all SUCCESS;
- retained direct control: **1,829** QIDs with **zero drift**;
- alternative discovery yield: R1 **+53**, R2 **+0**, R3 **+4**; alternative union **57**; all pairwise alternative-route overlaps **0**;
- deterministic entity-detail sample: **57/57** acquired, within <=75/route and <=200-global caps;
- exact identity-signal categories: **0** accepted-QID overlap / **0** exact-other-QID / **57** no-exact-signal / **0** unresolved-exact-signal;
- dispositions: R1/R3 `FOLLOWUP_DISCOVERY_CANDIDATE`, R2 `NO_INCREMENTAL_YIELD`, recommendations only;
- amendment round 1 hardened structural/offline verification and live sample completeness and corrected CI-provenance wording;
- amendment round 2 added exact full-set verification for drift/incremental/overlap QID sets and sampled accepted-universe references;
- retained live measurement was not rerun or altered during either amendment round;
- no canonical Brand/Organization/BoatModel/BoatDesign rows created or modified;
- no HullQ IDs minted and no production Wikidata discovery query changed;
- explicit project-owner acceptance on 2026-08-24.

Final closure record: `docs/slices/SLICE-0021-acceptance-closure.md`.

No SLICE-0022 or later slice was made `READY` by the SLICE-0021 closure itself; SLICE-0022 was authorized separately by its readiness decision and is now accepted / `DONE`.
...
alternative Wikidata class-discovery semantics    DONE — SLICE-0021
        ↓
retained alternative-route Tier-0 admission       DONE — SLICE-0022
        ↓
...
SLICE-0019 is closed and accepted `DONE` (see `docs/slices/SLICE-0019-acceptance-closure.md`). SLICE-0020 is closed and accepted `DONE` (see `docs/slices/SLICE-0020-acceptance-closure.md` and `research/manufacturers/archive_clearance/ARCHIVE_SOURCE_CLEARANCE_REPORT.md`). SLICE-0021 is closed and accepted `DONE` (see `docs/slices/SLICE-0021-acceptance-closure.md` and `research/bootstrap/wikidata/sl0021-alt-discovery/REPORT.md`). SLICE-0022 is closed and accepted `DONE` (see `docs/slices/SLICE-0022-acceptance-closure.md` and `research/bootstrap/wikidata/sl0022-alt-route-admission/REPORT.md`). SLICE-0023 was made `READY` by its own separate readiness decision (see the slice document's "Readiness authority note") and its implementation is now complete and handed off as `REVIEW`, pending independent review and project-owner acceptance (see `research/bootstrap/wikimedia/sl0023-category-leads/REPORT.md`). No SLICE-0024 or later slice has been created or made `READY`.
