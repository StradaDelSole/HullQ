"""End-to-end runner-level tests for
scripts/bootstrap/wikidata_sl0022_alt_route_admission_runner.py.

All tests are offline and deterministic by construction: SLICE-0022 performs
zero network access in any mode. ``run_classify``/``run_verify`` are
exercised against the real committed immutable retained inputs (SLICE-0017/
0018/0021), writing their own manifest/report output to ``tmp_path`` so the
real committed
``research/bootstrap/wikidata/sl0022-alt-route-admission/manifest.json`` is
never touched by tests.
"""

from __future__ import annotations

import json
import sys
from pathlib import Path

import pytest

ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT / "scripts"))

from bootstrap.wikidata_sl0022_alt_route_admission_runner import (  # noqa: E402
    run_classify,
    run_verify,
)

from hullq.bootstrap.wikidata_sl0022_alt_route_admission import (  # noqa: E402
    EXPECTED_R1_COUNT,
    EXPECTED_R3_COUNT,
    EXPECTED_TOTAL_CANDIDATES,
)


def test_run_classify_writes_valid_manifest_and_report(tmp_path: Path) -> None:
    manifest_path = tmp_path / "manifest.json"
    report_path = tmp_path / "REPORT.md"

    manifest = run_classify(manifest_path=manifest_path, report_path=report_path)

    assert manifest_path.exists()
    assert report_path.exists()
    assert manifest["candidate_universe"] == {
        "total": EXPECTED_TOTAL_CANDIDATES,
        "r1_count": EXPECTED_R1_COUNT,
        "r2_count": 0,
        "r3_count": EXPECTED_R3_COUNT,
    }
    assert manifest["counts"]["auto_admit_r3"] == 0
    assert manifest["counts"]["candidates_processed"] == EXPECTED_TOTAL_CANDIDATES
    assert manifest["acquired_at"] == manifest["generated_at"]
    assert manifest["classification_recomputed_at"] is None

    report_text = report_path.read_text(encoding="utf-8")
    assert "SLICE-0022" in report_text
    assert "ZERO LIVE NETWORK ACQUISITION" in report_text


def test_run_classify_is_idempotent_across_reruns(tmp_path: Path) -> None:
    manifest_path = tmp_path / "manifest.json"
    report_path = tmp_path / "REPORT.md"

    first = run_classify(manifest_path=manifest_path, report_path=report_path)
    second = run_classify(manifest_path=manifest_path, report_path=report_path)

    first_ids = {row["qid"]: row["hullq_id"] for row in first["candidates"] if row["hullq_id"]}
    second_ids = {row["qid"]: row["hullq_id"] for row in second["candidates"] if row["hullq_id"]}
    assert first_ids == second_ids  # every AUTO_ADMIT QID keeps its exact minted ID

    assert second["acquired_at"] == first["acquired_at"]  # preserved verbatim
    assert second["classification_recomputed_at"] is not None


def test_run_verify_passes_on_freshly_classified_manifest(tmp_path: Path) -> None:
    manifest_path = tmp_path / "manifest.json"
    report_path = tmp_path / "REPORT.md"
    run_classify(manifest_path=manifest_path, report_path=report_path)

    run_verify(manifest_path=manifest_path)  # must not raise


def test_run_verify_fails_closed_on_tampered_manifest(tmp_path: Path) -> None:
    manifest_path = tmp_path / "manifest.json"
    report_path = tmp_path / "REPORT.md"
    run_classify(manifest_path=manifest_path, report_path=report_path)

    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    row = next(r for r in manifest["candidates"] if r["route_membership"] == ["R3"])
    row["decision"] = "auto_admit"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

    with pytest.raises(SystemExit):
        run_verify(manifest_path=manifest_path)


def test_run_verify_passes_against_real_committed_manifest() -> None:
    """The actual retained SLICE-0022 manifest committed to the repository
    must itself pass offline verification — the same check normal CI runs.
    """
    from bootstrap.wikidata_sl0022_alt_route_admission_runner import MANIFEST_PATH

    run_verify(manifest_path=MANIFEST_PATH)  # must not raise
