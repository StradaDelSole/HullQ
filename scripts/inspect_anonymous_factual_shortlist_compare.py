...
`specs/ANONYMOUS_FACTUAL_SHORTLIST_COMPARE_CONTRACT.v0.1.md` §15:
    ...
    13. after a server-side offer price revision and a PhysicalBoat claim
        ...
        the exact same retained shortlist payload from step 12 -- reseeded
        verbatim, never regenerated by calling `addToShortlist()` again --
        is what carries through the revision boundary
...
if (mode === "seed") {
  ...
  // Independent re-review 2026-09-19 (PR #221, residual B): seed storage
  // with the EXACT raw bytes retained from the prior "seed" run's
  // raw_before -- never call addToShortlist() again -- so this run
  // genuinely carries the same stored payload through the server-side
  // truth change instead of merely regenerating an equivalent one.
  storage.setItem(store.SHORTLIST_STORAGE_KEY, seedRawPayload);
} else {
...
def _run_store_sourced_harness(
    web_base: str, *, mode: str, seed_raw_payload: str = ""
    ...
    *mode* is `"seed"` (build local membership via real `addToShortlist`
    calls, buyer-authored order) or `"reuse"` (seed storage with the EXACT
    *seed_raw_payload* bytes retained from a prior `"seed"` run's
    `raw_before`, never calling `addToShortlist` again -- residual B,
    2026-09-19: proves the same stored payload is carried through a
    server-side truth change rather than an equivalent one being
    regenerated).
    """
...
def main() -> int:
    ...
    try:
        ...
        # 12/13 (independent review 2026-09-19, PR #221, blocker 3; residual
        # B fixed 2026-09-19): prove the compare set is sourced from the
        # real shipped shortlist store in buyer-authored order (not a
        # hand-typed literal), that resolving through Compare's own runtime
        # seam leaves local membership byte-identical, and that the exact
        # same retained payload -- not an equivalent one regenerated by
        # calling addToShortlist() again -- is carried through a later
        # server-side offer/claim revision while the next resolution
        # reflects the new current values.
        before_ok, before = _run_store_sourced_harness(web_base, mode="seed")
        ...
        # Residual B (independent re-review 2026-09-19, PR #221): re-seed
        # storage with the EXACT `raw_before` bytes step 12 already observed
        # -- never call addToShortlist() again -- so this run genuinely
        # carries the same stored shortlist payload through the server-side
        # truth change rather than regenerating an equivalent one and only
        # then comparing bytes.
        retained_raw_payload = before.get("raw_before")
        assert isinstance(retained_raw_payload, str)
        after_ok, after = _run_store_sourced_harness(
            web_base, mode="reuse", seed_raw_payload=retained_raw_payload
        )
        step_store_after_ok = (
            after_ok
            and after.get("ids") == before.get("ids")
            and after.get("raw_before") == retained_raw_payload
        ...
        print(
            "13. the exact same retained shortlist payload (not regenerated) is carried through a "
            "later server-side offer/claim revision, and the next resolution reflects both new "
            f"current values -> {'OK' if step_store_after_ok else 'FAIL'}\n"
        )
