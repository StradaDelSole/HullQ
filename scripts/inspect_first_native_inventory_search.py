...
servers (FastAPI + built Astro/Node SSR):

    1. Alembic from a clean schema through the current head.
    2. Seed two real BoatDesigns (one with a shallow-draft NamedVariant
       override making it draft_max-compatible with 1.6 m, one always too
       deep) plus five ACTIVE NativeListings and one DRAFT (unpublished)
       listing, each with its own publishing Organization and PhysicalBoat
       claim state: a confirmed shallow draft, an omitted draft, a
       too-deep design, a PhysicalBoat with no BoatDesignRef at all, and an
       unpublished (DRAFT) listing.
...
def main() -> int:
    ...
    try:
        ...
        print("1. Alembic migrated to current head -> OK\n")
        ...
        try:
            ...
            print(
                "2. seeded 2 BoatDesigns + 4 ACTIVE listings (match/omitted/deep/no-identity) + "
                "1 DRAFT (unpublished) listing -> OK\n"
            )
        finally:
            conn.close()
