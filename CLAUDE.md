...
- For recurring local diagnostics, prefer `uv run python scripts/workflow/claude_diag.py` rather than composing shell. Supported helpers include `env-status <NAME>`, `tcp-check <HOST> <PORT>`, `latest-temp-dir <PREFIX>`, `run-local-test-db <scripts/...py>`, and `run-local-api [--host ... --port ... --seconds ...]`. Never prefix commands with `HULLQ_TEST_DATABASE_URL=...`, `HULLQ_DATABASE_URL=...`, or `HULLQ_PREVIEW_SIGNING_SECRET=...`; the helper injects the fixed local-test values only into the child process without printing them.
